...
import com.daisy.bangsen.dao.CustomerDao;
...
public class CustomerInfoServiceImpl implements CustomerInfoService {
    ...
    public RespBean save(String postData) {
        ...
        try {
            CustomerInfo customerInfo = JSONUtil.toBean(postData, CustomerInfo.class);
            int re = customerDao.insert(customerInfo);
            if (re > 0) {
    ...
    public RespBean delete(String id) {
        RespBean respBean=new RespBean();
        try {
            int re=customerDao.deleteById(JSONUtil.parseObj(id).getStr("id"));
            if (re>0){
                respBean.setStatus(200);
    ...
    public RespBean update(String postData) {
        ...
        try {
            CustomerInfo customerInfo= JSONUtil.toBean(postData,CustomerInfo.class);
            int re=customerDao.updateById(customerInfo);
            if (re>0){
                respBean.setStatus(200);
    ...
    public RespBean query(String postData) {
        ...
        try {
            ...
            if (jsondata.containsKey("name") && StringUtils.isNotBlank(jsondata.getStr("name"))) {
                paraMap.put("customer_name", jsondata.get("name"));
            }
            ...
            if (StringUtils.isBlank(jsondata.get("currentpage").toString()) && StringUtils.isBlank(jsondata.get("pagesize").toString())) {
                pageBean = new Page<CustomerInfo>(1, 10);
            } else {
                pageBean = new Page<CustomerInfo>(Long.parseLong(jsondata.getStr("currentpage")), Long.parseLong(jsondata.getStr("pagesize")));
            }
            paraMap.put("page", pageBean);
            List re = customerDao.selectByParam(paraMap);
            int allsize = customerDao.selectCount(null);
